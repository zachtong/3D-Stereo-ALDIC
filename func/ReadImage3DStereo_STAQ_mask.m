function [full_path_0, full_path_1, Img_0, Img_1, LoadImgMethod] = ReadImage3DStereo_STAQ_mask(imageLeft,imageRight)
%FUNCTION [file_name,Img,DICpara] = ReadImage3DStereo(varargin)
% ==============================================
fprintf('\n Choose method to load MASKS:  \n')

fprintf('     0: Select mask folder;  \n')
fprintf('     1: Separately select LEFT and RIGHT mask folders;  \n')
fprintf('     2: Draw a masks on the first left and right raw images. \n')
prompt = 'Input here: ';
...
switch LoadImgMethod
    ...
    case 2 % Directly draw a mask on the left and right raw images
        % Left mask drawing
        Img_0{1} = createMask(imageLeft); % figure; imshow(maskLeft)
        % Right mask drawing
        Img_1{1} = createMask(imageRight);
